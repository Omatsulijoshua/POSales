...
namespace POSales
{
    public partial class StockIn : Form
    {
        ...
        public void LoadStockIn()
        {
            ...
            {
                i++;
                dgvStockIn.Rows.Add(i, dr[0].ToString(), dr[1].ToString(), dr[2].ToString(), dr[3].ToString(), dr[4].ToString(), FormatDateTime(dr[5]), dr[6].ToString(), dr["supplier"].ToString());
                ...
                if (dgvStockIn.Rows.Count > 0)
                {
                    if (MessageBox.Show("Are you sure you want to save this records?", stitle, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        for(int i=0; i<dgvStockIn.Rows.Count;i++)
                        {
                            ...
                            cn.Open();
                            DateTime stockInDateTime = dtStockIn.Value.Date + DateTime.Now.TimeOfDay;
                            cm = new SqlCommand("UPDATE tbStockIn SET qty = qty + @qty, status='Done', sdate=@sdate WHERE id LIKE @id", cn);
                            cm.Parameters.AddWithValue("@qty", int.Parse(dgvStockIn.Rows[i].Cells[5].Value.ToString()));
                            cm.Parameters.AddWithValue("@sdate", stockInDateTime);
                            cm.Parameters.AddWithValue("@id", dgvStockIn.Rows[i].Cells[1].Value.ToString());
                            cm.ExecuteNonQuery();
                            cn.Close();
                        }
                        Clear();
        ...
        private void btnLoad_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    i++;
                    dgvInStockHistory.Rows.Add(i, dr[0].ToString(), dr[1].ToString(), dr[2].ToString(), dr[3].ToString(), dr[4].ToString(), FormatDateTime(dr[5]), dr[6].ToString(), dr["supplier"].ToString());
