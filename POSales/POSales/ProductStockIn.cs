...
namespace POSales
{
    public partial class ProductStockIn : Form
    {
        ...
        public void addStockIn(string pcode)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(stockIn.lblId.Text) ||
                    !int.TryParse(stockIn.lblId.Text, out int supplierId))
                {
                    MessageBox.Show("Invalid Supplier ID", stitle, MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }

                cn.Open();
                cm = new SqlCommand(@"INSERT INTO tbStockIn 
            (refno, pcode, sdate, stockinby, supplierid)
            VALUES (@refno, @pcode, @sdate, @stockinby, @supplierid)", cn);

                cm.Parameters.AddWithValue("@refno", stockIn.txtRefNo.Text);
                cm.Parameters.AddWithValue("@pcode", pcode);
                DateTime stockInDateTime = stockIn.dtStockIn.Value.Date + DateTime.Now.TimeOfDay;
