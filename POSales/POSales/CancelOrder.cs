    ...
    public partial class CancelOrder : Form
    {
        ...
        private void btnCOrder_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    if (!cboInventory.Text.Equals("Yes", StringComparison.OrdinalIgnoreCase))
                    {
                        MessageBox.Show("Unable to void this transaction. Add To Inventory must be Yes before cancelling.", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        return;
                    }
