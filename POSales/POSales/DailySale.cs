...
namespace POSales
{
    public partial class DailySale : Form
    {
        ...
        private void EnsurePaymentTypeGridColumn()
        {
            if (dgvSold.Columns.Contains("PaymentType"))
                return;

            DataGridViewTextBoxColumn column = new DataGridViewTextBoxColumn();
            column.Name = "PaymentType";
            column.HeaderText = "Payment Type";
            column.AutoSizeMode = DataGridViewAutoSizeColumnMode.AllCells;
            column.MinimumWidth = 105;

            int cancelIndex = dgvSold.Columns.Contains("Cancel") ? dgvSold.Columns["Cancel"].Index : dgvSold.Columns.Count;
            dgvSold.Columns.Insert(cancelIndex, column);
        }
        ...
        public void LoadSold()
        {
            ...
            DateTime dateTo = dtTo.Value.Date.AddDays(1);
            string cashierFilter = string.IsNullOrWhiteSpace(cboCashier.Text) || cboCashier.Text == "Select Cashier"
                ? "All Cashier"
                : cboCashier.Text;

            if(cashierFilter=="All Cashier")
            {
                cm = new SqlCommand("select c.id, c.transno, c.pcode, p.pdesc, c.price, c.qty, c.disc, c.total, ISNULL(c.paymenttype, 'Cash') as paymenttype from tbCart as c inner join tbProduct as p on c.pcode = p.pcode where status = 'Sold' and sdate >= @dateFrom and sdate < @dateTo", cn);
            }
            else
            {
                cm = new SqlCommand("select c.id, c.transno, c.pcode, p.pdesc, c.price, c.qty, c.disc, c.total, ISNULL(c.paymenttype, 'Cash') as paymenttype from tbCart as c inner join tbProduct as p on c.pcode = p.pcode where status = 'Sold' and sdate >= @dateFrom and sdate < @dateTo and cashier = @cashier", cn);
                cm.Parameters.AddWithValue("@cashier", cashierFilter);
            }
            ...
            while(dr.Read())
            {
                ...
                dgvSold.Rows.Add(i, dr["id"].ToString(), dr["transno"].ToString(), dr["pcode"].ToString(), dr["pdesc"].ToString(), dr["price"].ToString(), dr["qty"].ToString(), dr["disc"].ToString(), dr["total"].ToString(), dr["paymenttype"].ToString());
            }
            dr.Close();
            cn.Close();
